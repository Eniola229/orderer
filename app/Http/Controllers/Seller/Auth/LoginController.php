    ...
    public function login(Request $request)
    {
        ...
        if (!$seller->is_active) {
            ...
            return back()->withErrors([
                'email' => 'Your seller account has been suspended. Contact support@ordererweb.com'
            ]);
        }
        ...
        try {
            app(\App\Services\BrevoMailService::class)
                ->sendLoginNotification($seller, $request->ip(), $request->userAgent(), 'seller');
        } catch (\Exception $e) {
            \Log::error('Login email failed (seller): ' . $e->getMessage());
        }

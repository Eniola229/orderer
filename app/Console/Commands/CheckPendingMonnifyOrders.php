...
class CheckPendingMonnifyOrders extends Command
{
    protected $signature   = 'monnify:check-pending-orders';
    protected $description = 'Check pending Monnify orders and update payment status';
    ...
    {
        $pendingOrders = Order::where('payment_method', 'monnify')
        ...
        if ($pendingOrders->isEmpty()) {
            $this->info('No pending Monnify orders found.');
            return self::SUCCESS;
        }
        ...
        foreach ($pendingOrders as $order) {
            try {
                $data          = $monnify->verifyTransaction($order->payment_reference);
                // Monnify uses 'paymentStatus' with uppercase values: PAID, OVERPAID,
                // PARTIALLY_PAID, PENDING, FAILED, EXPIRED, REVERSED, CANCELLED
                ...
                if (in_array($monnifyStatus, ['PAID', 'OVERPAID'])) {
                    ...
                    // PENDING, PARTIALLY_PAID, or any unknown status — leave it alone
                    $skipped++;
                }

            } catch (\Exception $e) {
                $errors++;
                \Log::error("monnify:check-pending-orders — order #{$order->order_number}: " . $e->getMessage());
            }
    ...
    protected function bookShipmentForOrder(Order $order): void
    {
        ...
        foreach ($sellerGroups as $sellerId => $sellerItems) {
            ...
            if (!$courierId) {
                \Log::warning("checkPendingMonnify — skipping seller {$sellerId}: missing courier_id");
                continue;
            }
            ...
            try {
                ...
                \Log::error("checkPendingMonnify — shipment failed for seller {$sellerId} order #{$order->order_number}: " . $e->getMessage());
            }
        }
    }

    // ── Mirrors CheckoutController::sendOrderEmails() ────────────────────────
    protected function sendOrderEmails(Order $order): void
    {
        try {
            ...
            \Log::error("checkPendingMonnify — sendOrderEmails failed for order #{$order->order_number}: " . $e->getMessage());
        }
    }
